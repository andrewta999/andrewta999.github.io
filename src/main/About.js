...
export default function About() {
  const skill = skills.map(({ name, skill }) => {
    let list_skill = skill.map(({ name, icon }) => {
      return (
        <div key={name} className="About-tech">
          <span className="About-icon" key={`${name}-icon`}>
            {icon}
          </span>
          <span className="About-name" key={`${name}-name`}>
            {name}
          </span>
        </div>
      );
    ...
    return (
      <div
        className="col-12 col-sm-12 col-md-6 col-lg-3 col-xl-3 About-col"
        key={name}
      >
  ...
  return (
    <div className="About">
      <div className="container">
        ...
        <div className="About row">{skill}</div>
      </div>
    </div>
  );
}
